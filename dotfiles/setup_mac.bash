...
# load_struct_testing_wget
  ...
  echo "$BASH_VERSION" |  sed -E 's/[a-z-]|[A-Z]|\)|\(//g'
  echo $BASH_VERSION | cut -f1 -d.

function _prepare(){
  echo "  loads
   execute_command_intuivo_cli/execute boot basic
     that loads
   execute_as_sudo.sh:execute_as_sudo
   task_intuivo_cli/add_error_trap.sh:_trap_on_error
   execute_command_intuivo_cli/execute_command:_execute_command_worker
   execute_command_intuivo_cli/struct_testing:passed/execute_as_sudo.sh:execute_as_sudo
  "
  ...
  load_execute_boot_basic_with_sudo(){
    ...
    [ ! -e "${provider}"  ] && provider="https://raw.githubusercontent.com/zeusintuivo/execute_command_intuivo_cli/master/execute_boot_basic.sh"
    if ( command -v wget >/dev/null 2>&1; ) ; then
      echo "   wget --quiet --no-check-certificate "${provider}" -O -"
      [ ! -e "${provider}"  ] && eval """$(wget --quiet --no-check-certificate "${provider}" -O -  2>/dev/null )"""   # suppress only wget download messages, but keep wget output for variable
    elif ( command -v curl >/dev/null 2>&1; ); then
      echo "   curl "${provider}" -o -"
      [ ! -e "${provider}"  ] && eval """$(curl "${provider}" -o -  2>/dev/null )"""   # suppress only curl download messages, but keep curl output for variable
    else
    ...
    {
      echo -e "\n \n  ERROR! execute_boot_basic.sh did not load.  \n \n "
      exit 1;
    }
    fi
    return 0
...
function mac_general_config(){
if [ $1 ]; then
	echo ""
	echo "HELP:"
	echo "export SSH_KEYS=1 to install SSH-Keys."
	echo ""
	exit
fi

echo ""
echo "Setting default configuration preferences..."
echo ""
...
echo "Graphite appearance";
defaults write -g 'AppleAquaColorVariant' -int 6;

echo "Graphite highlight color";
defaults write -g 'AppleHighlightColor' -string '0.780400 0.815700 0.858800';

echo "Disable new document window animations";
defaults write NSGlobalDomain NSAutomaticWindowAnimationsEnabled -bool NO;

echo "Speed up sheet animation";
defaults write NSGlobalDomain NSWindowResizeTime .1;

echo "Recent applications to 50";
osascript -e 'tell application "System Events" to tell appearance preferences to set recent applications limit to 50';

echo "Recent documents to 50";
osascript -e 'tell application "System Events" to tell appearance preferences to set recent documents limit to 50';

echo "Recent servers to 50";
osascript -e 'tell application "System Events" to tell appearance preferences to set recent servers limit to 50';

echo "Make scroll direction not-natural (scrolls the direction of finger movement)";
defaults write -g com.apple.swipescrolldirection -bool false;

echo "Disable keyboard press and hold (in favor of repeat)";
defaults write NSGlobalDomain ApplePressAndHoldEnabled -bool false;

echo "Make key-repeat rate faster";
defaults write KeyRepeat -float 2.5;

echo "Make delay until key-repeat shorter";
defaults write InitialKeyRepeat -float 10;

echo "Enable full keyboard access for all controls";
defaults write NSGlobalDomain AppleKeyboardUIMode -int 3;

echo "Dock tile size 48";
defaults write com.apple.dock 'tilesize' -int 48;

echo "Dock magnification on";
defaults write com.apple.dock 'magnification' -bool true;

echo "Dock large size 110";
defaults write com.apple.dock 'largesize' -int 110;

echo "Auto hide the dock";
defaults write com.apple.dock autohide -bool true;

echo "Set dock show animation delay to 0";
defaults write com.apple.dock autohide-delay -float 0;

echo "Enable accessibility zoom";
defaults write com.apple.universalaccess 'closeViewScrollWheelToggle' -bool true;
defaults write com.apple.universalaccess 'HIDScrollZoomModifierMask' -int 262144;

echo "Smooth images for accessibility zooming";
...
echo "Disable save to icloud by default in save panel";
defaults write NSGlobalDomain NSDocumentSaveNewDocumentsToCloud -bool false;

echo "New windows open home folder in Finder";
...
echo "Disable warning for file exension changes in Finder";
...
echo "Enable Safari's Develop and Web Inspector";
defaults write com.apple.Safari IncludeDevelopMenu -bool true;
defaults write com.apple.Safari WebKitDeveloperExtrasEnabledPreferenceKey -bool true;
defaults write com.apple.Safari "com.apple.Safari.ContentPageGroupIdentifier.WebKit2DeveloperExtrasEnabled" -bool true;
defaults write NSGlobalDomain WebKitDeveloperExtras -bool true;

echo "Disable back-swipe in Chrome";
...
echo "Set terminal theme to Pro";
#defaults write com.apple.Terminal "Default Window Settings" -string "Pro";
#defaults write com.apple.Terminal "Startup Window Settings" -string "Pro";

echo "Stop Time Machine from prompting to use new hard drives as backup volume";
defaults write com.apple.TimeMachine DoNotOfferNewDisksForBackup -bool true;

echo "Disable OS X Gate Keeper (allow application install from anywhere)";
...
echo "Enable debug menu in Disk Utility";
defaults write com.apple.DiskUtility 'DUDebugMenuEnabled' -bool true;

echo "Enable developer mode for CrashReporterPrefs"
defaults write com.apple.CrashReporter 'DialogType' -string 'developer'

}
echo "Install Mac General Config Preferences"
# mac_general_config
# exit 0
function install_ssh_keys(){
  ...
  {
    echo ""
    echo "Checking for SSH-Keys.dmg..."
    if [ -x /Volumes/SSH-Keys/install.sh ]; then
    {
      echo "  Running /Volumes/SSH-Keys/install.sh"
      CWD=`pwd`
      cd /Volumes/SSH-Keys/
      ./install.sh
      cd $CWD
...
# exit 0
function say_webssites(){
  echo ""
  echo "Software Websites:"
  echo ""
  echo "https://www.google.com/chrome/browser/"
  echo "https://www.mozilla.org/en-US/firefox/new/"
  echo "http://www.sublimetext.com/3"
  echo "http://www.macromates.com/"
  echo "http://www.sourcetreeapp.com/"
  echo "http://www.trankynam.com/xtrafinder/"
  echo "http://www.charlesproxy.com/download/"
  echo "https://adium.im/"
  echo "http://panic.com/transmit/"
  echo "http://www.alfredapp.com/"
  echo "http://creative.adobe.com/"
  echo "http://colloquy.info/downloads.html"
  echo "http://www.videolan.org/vlc/download-macosx.html"
  echo "https://www.transmissionbt.com/download/"
  echo "https://justgetflux.com/"
  echo "http://www.delibarapp.com/"
  echo "https://handbrake.fr/downloads.php"
  echo "https://www.crashplan.com/account/login.vtl"
  echo "http://terminal.sexy/"

  echo ""
  echo "Sublime Stuff:"
  echo "https://sublime.wbond.net/"
  echo "https://sublime.wbond.net/packages/Theme%20-%20Gravity"
  echo "https://github.com/AndreasVerhoeven/NoMailAppAnimationsOnMavericks"
  echo "https://github.com/netatoo/phoenix-theme"
  echo "https://github.com/Twiebie/ST-Spacefunk"
  echo "https://github.com/chriskempson/base16"
  echo "https://github.com/skuroda/PackageResourceViewer"
}
# exit 0
...
else
{
  ...
  _run_command "/bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\" "
}
fi
...
# exit 0
        function check_to_be_installed() {
            ...
            {
              # CHECK_INSTALLED=$(brew ls --versions "${package}")
              CHECK_INSTALLED=$(brew ls --versions "${package}")
              ...
              {
                echo -e "${YELLOW} Installing ${message}   ${CERO}"
                 _run_command "brew install \"${package}\" -y"
              }
              fi
...
# check_to_be_installed "ls"
# exit 0
function install_requirements() {
  function _trap_on_exit(){
    echo -e "\033[01;7m*** TRAP $THISSCRIPTNAME ERR or EXITS ...\033[0m"
  ...
  trap _trap_on_exit EXIT
            ...
            for one_required in ${required}; do
            {
              ...
              {
                packages=$(rpm -qa | egrep "$one_required" | cut -d' ' -f3 2>&1)
                query=$(rpm-query -W -f='${Status} ${Version}\n' "$one_required"  2>&1)
                #if [[ "${query}" == "install ok"* ]] && ( is_included "$one_required" "${packages}" ); then
                if [[ "${query}" == "install ok"* ]] ; then
                {
                    passed " ${GREEN} $one_required ${YELLOW} INSTALLED${RESET} ."
              ...
              elif ( command -v brew >/dev/null 2>&1; ) ; then
              {
                ...
                query=$(su - "${SUDO_USER}" -c "brew list --formula")
                # query=$(_run_command "brew list -1")
                if [[ "${query}" == "${one_required}*" ]] ; then
                {
                    passed " ${GREEN} $one_required ${YELLOW} INSTALLED${RESET} ."
                }
                else
                {
                    ...
                    echo -e "${YELLOW} Installing ${message}   ${CERO}"
                    missing="${missing} ${one_required}"
                    su - "${SUDO_USER}" -c "brew install \"${one_required}\" -y"
                    # _run_command "brew install \"${one_required}\" -y"
                }
                fi
              }
              fi
...
echo "Install Node.js with nodenv"
...
echo "Install Perl 5 with plenv"
...
# Iris.dmg|https://raw.githubusercontent.com/danielng01/product-builds/master/iris/macos/Iris-1.2.0-OSX.zip
# 1Password.pkg|https://c.1password.com/dist/1P/mac7/1Password-7.7.pkg

installlist="
Brave-Browser.dmg|https://referrals.brave.com/latest/Brave-Browser.dmg
Firefox.dmg|https://download-installer.cdn.mozilla.net/pub/firefox/releases/84.0.2/mac/de/Firefox%2084.0.2.dmg
MFF2_latest.dmg|http://mff.s3.amazonaws.com/MFF2_latest.dmg
Caffeine.dmg|https://github.com/IntelliScape/caffeine/releases/download/1.1.3/Caffeine.dmg
Vlc.dmg|https://download.vlc.de/vlc/macosx/vlc-3.0.11.dmg
Keybase|https://prerelease.keybase.io/Keybase.dmg
"
...
while read -r one ; do
{
  if [[ -n "${one}" ]] ; then
  {
    ...
    target_url="${echo "${one}" | cut -d'|' -f2)
    _install_dmg__64 "${target_name}" "${target_url}"
  }
  fi
}
done <<< "${installlist}"

echo "Restart or logout for settings to take effect.";
